...
const Formulas = {
  ...
  getELong(length, width, height, mass, peakFreqLong) {
    ...
    const eLong =
      ((mathConst * density * Math.pow((length * milli), 4) * Math.pow(peakFreqLong, 2)) /
  ...
  getECross(length, width, height, mass, peakFreqCross) {
    const density = this.getDensity(length, width, height, mass);
    const eCross =
      ((mathConst * density * Math.pow((width * milli), 4) * Math.pow(peakFreqCross, 2)) /
        Math.pow((height * milli), 2))
        * nano;
  ...
  getVelocitySoundLong(length, width, height, mass, eLong) {
    const density = this.getDensity(length, width, height, mass);
    const velocitySoundLong = 
    Math.sqrt((eLong * giga) / density) * milli
